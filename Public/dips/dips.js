// Fetches dips from /api/products?category=dips and renders them into
// #dips-grid, then wires the same add-to-cart interaction as before
...
async function loadDips() {
  try {
    const res = await fetch('/api/products?category=dips');
    const products = await res.json();

    if (products.length === 0) {
      grid.innerHTML = '<p class="grid-empty">No dips available right now.</p>';
      return;
    }
