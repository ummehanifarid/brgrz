// Fetches beverages from /api/products?category=beverages and renders them into
// #beverage-grid, then wires the same add-to-cart interaction as before
...
async function loadBeverages() {
  try {
    const res = await fetch('/api/products?category=beverages');
    const products = await res.json();

    if (products.length === 0) {
      grid.innerHTML = '<p class="grid-empty">No beverages available right now.</p>';
      return;
    }
