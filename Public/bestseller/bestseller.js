// Add-to-cart interaction:
  // - Variant items (sizes / Half-Full): button stays disabled until a variant is picked.
  ...
  document.querySelectorAll('.item-card').forEach(card => {
    const variantBtns = card.querySelectorAll('.variant-btn:not(.variant-na)');
    const addBtn = card.querySelector('.add-cart-btn');
    let selectedBtn = null;

    if (variantBtns.length > 0) {
      // Variant-based item (sizes / Half-Full)
